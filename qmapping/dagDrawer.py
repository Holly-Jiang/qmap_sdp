    ...
    if "DAGDependency" in type_str:
        ...
        def node_attr_func(node):
            ...
            if style == "color":
                ...
                print("Unrecognized style %s for the dag_drawer." % style)
        edge_attr_func = None
            ...
            if style == "color":
                ...
                if isinstance(node, DAGInNode):
                    if isinstance(node.wire, Qubit):
                        label = register_bit_labels.get(node.wire, f"q_{node.wire.index}")
                    else:
                        label = register_bit_labels.get(node.wire, f"c_{node.wire.index}")
                    n["label"] = label
                    n["color"] = "black"
                    n["style"] = "filled"
                    n["fillcolor"] = "green"
                if isinstance(node, DAGOutNode):
                    if isinstance(node.wire, Qubit):
                        label = register_bit_labels.get(node.wire, f"q[{node.wire.index}]")
                    else:
                        label = register_bit_labels.get(node.wire, f"c[{node.wire.index}]")
                    n["label"] = label
                    n["color"] = "black"
                    n["style"] = "filled"
                    n["fillcolor"] = "red"
        ...
        def edge_attr_func(edge):
            ...
            if isinstance(edge, Qubit):
                label = register_bit_labels.get(edge, f"q_{edge.index}")
            else:
                label = register_bit_labels.get(edge, f"c_{edge.index}")
            e["label"] = label
            return e
